            ...
            VStack(spacing: 20) {
                ...
                HStack(spacing: 15) {
                    ...
                    StatCard(
                        ...
                        value: "\(appState.immichService.getCompletedJobsToday().count)",
                ...
                HStack(spacing: 15) {
                    ResourceMonitor(
                        title: "CPU Usage",
                        value: appState.serverStats?.cpuUsage ?? 0,
                        unit: "%",
                        icon: "cpu",
                        color: .blue
                    )
                    ...
                    ResourceMonitor(
                        title: "Memory Usage",
                        value: appState.serverStats?.memoryUsage ?? 0,
                        unit: "%",
                        icon: "memorychip",
                        color: .purple
                    )
...
struct ResourceMonitor: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            ProgressView(value: value, total: unit.isEmpty ? 100 : 100)
                .tint(color)
...
class DashboardViewModel: ObservableObject {
    ...
    private func updateProcessingRateMetrics(appState: AppState) {
        ...
        let metrics = appState.databaseManager.fetchMetrics(
            type: .completionRate,
            since: oneHourAgo
        )
        
        processingRateMetrics = metrics.map { metric in
            ProcessingRateMetric(timestamp: metric.timestamp, rate: metric.value)
        }
        
        // If we have current rate, add it
        if let rate = appState.serverStats?.averageProcessingRate {
            processingRateMetrics.append(ProcessingRateMetric(timestamp: now, rate: rate))
        }
    }
}
